...
public class ExcelExportService {
    ...
    public ByteArrayInputStream exportCategoriesToExcel(List<Category> categories) throws IOException {
        try (Workbook workbook = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            ...
            createCell(headerRow, 8, "Image Link", headerStyle);
            createCell(headerRow, 9, "Status", headerStyle);
    ...
    public ByteArrayInputStream exportProductsToExcel(List<Product> products) throws IOException {
        try (Workbook workbook = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            ...
            createCell(headerRow, 6, "Image Link", headerStyle);
            createCell(headerRow, 7, "Status", headerStyle);
            createCell(headerRow, 8, "Created Date", headerStyle);
            ...
            for (Product product : products) {
                ...
                createCell(row, 8, product.getCreatedDate() != null ? sdf.format(product.getCreatedDate()) : "", dateStyle);
